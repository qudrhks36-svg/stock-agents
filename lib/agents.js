...
function fmtChart(chart) {
  ...
  const lines = [
    ...
    chart.volAvg20 ? `최근 20일 평균 거래량: ${Math.round(chart.volAvg20).toLocaleString()}` : null,
    chart.psychoLine12 != null
      ? `투자심리선(최근 12거래일 중 상승 마감 비율): ${chart.psychoLine12.toFixed(0)}% ${
          chart.psychoLine12 >= 75 ? "(과열권, 단기 매수심리 과열)" : chart.psychoLine12 <= 25 ? "(침체권, 단기 매도심리 과열)" : "(중립)"
        }`
      : null,
    "",
...
다음 세 가지는 반드시 빠짐없이 언급해라 (생략 금지):
1. 이동평균선: 현재가가 5일선/20일선 대비 위인지 아래인지, 그 의미(정배열/역배열 등)
2. 거래량: 최근 5일 평균 거래량이 20일 평균 대비 늘었는지 줄었는지, 그게 무엇을 시사하는지
3. 투자심리선: 과열권/침체권/중립 중 어디인지, 개인투자자 심리가 어떤 상태인지

이 세 가지를 기반으로 추세(상승/하락/횡보)와 지지/저항 구간까지 판단해라.
